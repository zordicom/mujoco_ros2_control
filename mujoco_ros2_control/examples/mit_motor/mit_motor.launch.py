...
def generate_launch_description():
    ...
    urdf_path = os.path.join(pkg_share, "examples/mit_motor/mit_motor.urdf")
    with open(urdf_path, "r") as f:
        robot_description = f.read()

    config_path = os.path.join(pkg_share, "examples/mit_motor/mit_motor.yaml")
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[mit_controller_spawner],
            )
        ),
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=mit_controller_spawner,
                on_exit=[unpause_sim],
            )
        ),
    ])
